...
## v42.1 — 🎯 Anomaly gate: adjudication + evolve (A/B log · byte-identical OFF · calibration quality · drift) — 2026-07-02

**The adjudication the v42 entry above asked for — resolved by Steven, not silently.** A follow-up task arrived instructing a *from-scratch* rebuild of the gate against the Fable-5 doc (8-dim full-covariance), on the premise that the tree was "clean v26" and "the earlier attempt has been killed." **Both premises were false:** the 18-dim diagonal gate was the committed HEAD (`8fdbc52`), fully tested, working tree clean. Surfaced the contradiction instead of bulldozing a tested build on a bad premise. **Steven chose: evolve the committed diagonal gate, don't rip-and-replace.** The diagonal↔full-covariance question is unverifiable until real GPX 6000 audio anyway (the standing caveat), so a rewrite would buy churn, not proven value. Deferred the covariance-model swap to a real data bake-off once hardware audio exists.

### What v42.1 adds (all additive; the diagonal engine is untouched)
1. **Per-event A/B log** — every triggered event now carries `ev.gateLog = {timestamp, gate_score, gate_verdict, shadow_rms_value, spot_id, calibration_id}`. Gate-fired events log verdict `gate-fired`; manual hits log `manual-ungated` (never vetoed — the gate can only *add* auto-saves). `spot_id` is an ~11 m GPS grid cell (stable per-spot key, no new data model). Plus a **per-session shadow-RMS counter**: how many times the plain RMS trigger *would* have fired this session (counterfactual, no clip saved). Settings card shows `📊 gate captured N · amplitude would have captured M · NN% fewer`. Every field session in gate mode is now a free amp-vs-gate A/B.
2. **Truly byte-identical gate-OFF.** The old build ran a perpetual 200 ms `setTimeout` loop even with the gate off. Now the loop is **parked** (no timer scheduled) unless `anomalyGate && running` or a panel is open, and re-armed by the events that make it wanted (gate toggle, capture start, panel open). With the gate off + panels closed, **zero anomaly code executes** — the detection path is `onaudioprocess → feedRMS → maybeAutoFlag`, proven **byte-for-byte identical to v41.4** (`maybeAutoFlag` 696 B, `onaudioprocess` 226 B, both `diff`-identical v41.4 → HEAD → working tree).
3. **Calibration-quality reject/warn ladder** (`assessBaselineQuality`). Reject (refuse + retry): < 30 windows, or ≥ 3 flat/zero-variance features (muted mic / silence). Warn (save + ⚠ badge): unsteady ground (self-σ > 1.5 or P99 > 4σ), or an implausible noise floor. Verdict + warnings stored in `rec.meta`; surfaced in the calibration summary and baseline list.
4. **Drift detection (detect-don't-adapt).** Baselines now store calibration σ anchors `d2:{p50,p90,p99,max}`. A rolling ~2.5 min live-σ median above the baseline's own P90 raises a one-time "ground no longer matches — recalibrate?" banner. **The threshold never moves** — silent adaptation would blind the gate exactly where sensitivity matters.

### Verified ✓ (headless — `tools/anom_test.js`, 54/54)
- **Syntax gate:** `node --check` on all 15 inline `<script>` blocks — no parse breakage from the edits.
- **Math (unchanged engine, re-asserted):** probit, χ² survival (k=1,2,8,18), diagonal Mahalanobis (at-mean ≈ 0σ, far ≫ 4σ), baselineStats mean/(n−1)var, `_effVar` ridge.
- **A/B log shape:** exactly the 6 brief fields; gate_score rounding; manual fallback to live-σ + active baseline.
- **Dormancy truth table:** gate-off + no panel ⇒ parked (the byte-identity guarantee); gate-on + not-running ⇒ parked; panel-open ⇒ active.
- **Calibration ladder:** reject (few windows / flat features), warn (unsteady / loud floor), reject-beats-warn.
- **Byte-identity:** detection trigger path `diff`-identical to v41.4/v26.

### NOT yet verified (honest)
- **Live-browser run pending** — all 5 preview server slots were held by other chats this session, so the runtime confirmation (loop actually parking, gateLog populating end-to-end through real meyda + IndexedDB, in-app calibration reject, drift banner firing) is code-proven + Node-covered but not yet clicked through a real browser. The prior v42 build was preview-verified by its session; these changes are additive, guarded, and Node-covered on top of it.
- **Real GPX 6000 audio** — the standing caveat since v24/v26/v32 holds: nothing audio is "verified" until Steven's hardware test. Synthetic/bench gates the build; field gates the default. Gate still ships **OFF by default**.

Bumped APP_VERSION/SHELL_REV/SHELL_VERSION → v42.1.
